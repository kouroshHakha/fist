...
from spirl.models.bc_atomic import GoalBCMdl
...
class ModelTrainer(BaseTrainer):
    def __init__(self, args):
        ...
        train_params = AttrDict(logger_class=self._hp.logger,
                                ...
                                dataset_size=-1)
        self.logger, self.model_vae, self.train_loader = self.build_phase(train_params, 'train')
        ...
        self.logger_bc, self.model = self.build_bc_model()
    ...
    def eval(self):
        ...
        rst_points = np.load(self.conf.general['rst_data_path'])[:10]
        ...
        for i in range(10):
            ...
            for rst_idx, rst in enumerate(rst_points):
                ...
                states = [s]
                while True:
                    if isinstance(self.model, GoalBCMdl):
                        ...
                        out_dist = self.model._compute_output_dist(ar2ten(sg, self.device).float()[None])
                        a = ten2ar(out_dist.sample()[0])
                    else:
                        ipt = AttrDict({'states': torch.Tensor([[s]]).to(self.device).float()})
                        a = ten2ar(self.model(ipt).pred_act.sample())[0]
            ...
            summary.append(dict(success_rate=success_cnt / nrst, episode_lens=episode_lens,
                           rewards=rew_list,
    ...
    def train_epoch(self, epoch):
        self.model.train()
        train_loader = self.train_loader if self._hp.offline_data else self.fewshot_dloader
        ...
        for self.batch_idx, sample_batched in enumerate(train_loader):
            ...
            if self.log_outputs_now:
                ...
                print(('itr: {} Train Epoch: {} [{}/{} ({:.0f}%)]\tLoss: {:.6f}'.format(
                        self.global_step, epoch, self.batch_idx, len(train_loader),
                        100. * self.batch_idx / len(train_loader), losses.total.value.item())))
    ...
    def get_config(self):
        ...
        conf.bc_model = conf_module.bc_model
        ...
        conf.ckpt_path = conf.bc_model.checkpt_path if 'checkpt_path' in conf.bc_model else None
    ...
    def build_bc_model(self):
        if not self.args.dont_save:
            logger = self._hp.logger(self.log_dir, summary_writer=self.writer)
        else:
            logger = None

        if self.conf.general.bc_model is None:
            model = BCMdl(self.conf.bc_model, logger)
        else:
            model_cls = self.conf.general.bc_model
            model = model_cls(self.conf.bc_model, logger)

...
class ModelTrainer(BaseTrainer):
    def __init__(self, args):
        ...
        self.optimizer = self.get_optimizer_class()(filter(lambda p: p.requires_grad, self.model.p.parameters()), lr=self._hp.lr)
    ...
    def eval(self, replan_t):
        ...
        episode_lens = []
        imgs = []
        for rst_idx in range(1):
            print(f'Evaluating episode {rst_idx} ...')
            done = False
            episode_lens.append(0)
            success = False
            demo_set = self.conf.general['fewshot_data']
            inputs = demo_set.get_rolling_sub_trajectories(rst_idx)
            inputs = AttrDict(map_dict(lambda x: torch.from_numpy(x).float().to(self.device), inputs))
            outputs = self.model(inputs)

            actions = demo_set.seqs[0].actions
            states = demo_set.seqs[0].states
            s = env.reset()

            s = states[0]
            for z, s in zip(outputs.z, states):
                ipt = AttrDict({'states': torch.Tensor([[s]]).to(self.device)})
                a = self.model.decode(z[None, :], None, 1, ipt).cpu().detach().numpy()[0, 0]
                _, _, _, done = env.step(a)
                imgs.append(env.render(mode='rgb_array'))

            # for a in actions:
            #     s, _, _,  done = env.step(a)
            #     imgs.append(env.render(mode='rgb_array'))
            imageio.mimsave(video_path / 'eval_latent_out_dist.mp4', imgs, fps=25)
            breakpoint()
        #     while True:
        #         if isinstance(self.model, GoalClSPiRLMdl):
        #             demo_set = self.conf.general['fewshot_data']
        #             demos = next(iter(DataLoader(demo_set, batch_size=len(demo_set), shuffle=False)))
        #             demo_states = demos['states']
        #             dists = ((demo_states[:, 0] - s) ** 2).sum(axis=1)
        #             idx = np.argmin(dists)
        #             target = demo_states[idx][-1]
        #             sg = np.concatenate([s, target])
        #             z = self.model.compute_learned_prior(torch.Tensor([sg]).to('cuda')).sample()
        #         else:
        ...
        # summary = dict(success_rate=success_cnt / len(rst_points), episode_lens=episode_lens,

...
class ModelTrainer(BaseTrainer):
    ...
    def eval(self, replan_t):
        ...
        rst_points = np.load('data/maze/rsts.npy')[:10]
        import gym
        ...
        env.set_target(np.array([7, 1]) + env.np_random.uniform(low=-.1, high=.1, size=env.model.nq))
        env.reset()
        ...
        episode_lens = []
        imgs = []
        for idx, rst in enumerate(rst_points):
            s = rst
            ...
            while True:
                if isinstance(self.model, GoalClSPiRLMdl):
                    il_batch_size = self.conf['general']['il_demo_batch_size']
                    il_demo_states, il_demo_actions = self.conf['general']['il_demo_sampler'](il_batch_size)
                    dists = ((il_demo_states[:, 0] - s) ** 2).sum(axis=1)
                    idx = np.argmin(dists)
                    target = il_demo_states[idx][10]
                ...
                for i in range(replan_t):
                    ...
                    last_step = (idx == len(rst_points) - 1 and i == replan_t - 1)
                    if len(imgs) == max_videos and not video_done or last_step:
                        imageio.mimsave(video_path / 'eval.mp4', imgs, fps=120)
                        video_done = True

                    success = float(np.linalg.norm(s[0:2] - env.get_target()) <= 0.5)
                    success_cnt += success
    ...
    def train_epoch(self, epoch):
        ...
        for self.batch_idx, sample_batched in enumerate(self.train_loader):
            ...
            if 'il_demo_sampler' in self.conf['general']:
                il_batch_size = self.conf['general']['il_demo_batch_size']
                il_demo_states, il_demo_actions = self.conf['general']['il_demo_sampler'](il_batch_size)
                sample_batched['states'][-il_batch_size:] = torch.Tensor(il_demo_states)
                sample_batched['actions'][-il_batch_size:] = torch.Tensor(il_demo_actions)

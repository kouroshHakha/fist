...
SAVE_DIRECTORY = './skill_datasets'
# DEMOS_DIRECTORY = f'{os.environ["HOME"]}/Documents/dev/relay-policy-learning/selected_data'
DEMOS_DIRECTORY = f'{os.environ["HOME"]}/Documents/dev/relay-policy-learning/kitchen-mixed-top-bot-excluded'
DEMOS_SUBDIR_PATTERN = '*'
ENVIRONMENTS = ['kitchen-mixed-v0']
task_list = ['microwave', 'kettle', 'top burner', 'hinge cabinet']
...
def main():
    ...
    for demo_subdir in demo_subdirs:
        demo_files = glob.glob(os.path.join(demo_subdir, 'kitchen_relax*.pkl'))
    ...
    for env_name in ENVIRONMENTS:
        if task_list is not None:
            env = gym.make(env_name, task_elements=task_list).unwrapped
        else:
